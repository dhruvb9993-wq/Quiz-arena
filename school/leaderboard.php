<?php
/** QuizArena — School Admin: school leaderboard */
...
if (get('export') === 'csv') {
    export_csv('leaderboard.csv',
        ['Rank', 'Student', 'Username', 'Class', 'Quizzes Played', 'Total Score', 'Total Points', 'Coins Earned'],
        array_map(fn($r) => [$r['rank'], $r['full_name'], '@' . $r['username'], $r['class_name'] ?? '', $r['quizzes_played'], round($r['total_score'], 1), $r['total_points'], $r['coins_earned']], $rows));
}
...
<div class="dash-card">
    <div class="dash-card-header">
        <h5 class="dash-card-title">Rankings</h5>
        <a class="btn btn-sm btn-outline-success" href="?export=csv&period=<?= $period ?>&class=<?= $class_id ?>&quiz=<?= $quiz_id ?>"><i class="bi bi-file-earmark-spreadsheet"></i> CSV</a>
    </div>
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div><div class="fw-semibold small"><?= e($r['full_name']) ?></div><div class="fs-8 text-muted">@<?= e($r['username']) ?></div></div>
                        </div>
                    </td>
